 ...
 */
NS_LOG_COMPONENT_DEFINE ("MainEasy");
...
main (int argc, char *argv[])
{
  ...
  bool bgActive = false;
  uint16_t timer = 10;
  std::string ascii = "ascii.tr";
  std::string xml = "result.xml";
  ...
  cmd.AddValue("ascii", "Name of the ASCII tracer output file [default=ascii.tr] ",ascii);
  cmd.AddValue("xml", "Name of the flow monitor XML output file [default=result.xml] ",xml);
  cmd.AddValue("version", "Prefix of CSV output file [default=00] ",version);
  ...
  //Ipv4AddressHelper garbageAddress;
  //garbageAddress.SetBase ("177.0.0.0", "255.255.255.0");
  ...
  NodeContainer of;
  of.Add(backboneNodes.Get(0));
  of.Add(backboneNodes.Get(1));
  of.Add(backboneNodes.Get(2));
  of.Add(backboneNodes.Get(3));
  of.Add(backboneNodes.Get(4));
  of.Add(backboneNodes.Get(5));
  of.Add(backboneNodes.Get(6));
  of.Add(backboneNodes.Get(8));
  of.Add(backboneNodes.Get(21));
  of.Add(backboneNodes.Get(25));
  of.Add(backboneNodes.Get(28));
  of.Add(backboneNodes.Get(34));
  of.Add(backboneNodes.Get(35));
  of.Add(backboneNodes.Get(36));
  of.Add(backboneNodes.Get(37));
  of.Add(backboneNodes.Get(38));
  of.Add(backboneNodes.Get(39));
  of.Add(backboneNodes.Get(40));
  of.Add(backboneNodes.Get(41));
  of.Add(backboneNodes.Get(42));
  ...
  for(uint16_t i=0; i<of.GetN(); i++)
    lteHelper->ConnectSocketsIngress(of.Get(i), of.Get(i)->GetObject<Ipv4>()->GetAddress(1,0).GetLocal());
  ...
  if(singleMme) {
    for(uint16_t i=0; i<lteVoiceUeContainerHO.GetN(); i++)
    {
      ...
      lteHelper->S1HandoverRequestWithAnchorRelocation (Seconds(hoTime), lteVoiceUeContainerHO.Get(i),
    ...
    for(uint16_t i=0; i<lteVoiceUeContainerHO.GetN(); i++)
    {
      double hoTime = TriggerHO(radius, x->GetValue()) + 7.5;
      out << "handover UE " << lteVoiceUeContainerHO.Get(i)->GetId() << " at " << hoTime << " seconds" << endl;
      lteHelper->S1HandoverRequestWithAnchorAndMmeRelocation (Seconds(hoTime), lteVoiceUeContainerHO.Get(i),
                                    epcHelper2->GetUeDefaultGatewayAddress(), enbLteDevs.Get(0), enbLteDevs2.Get(0));
    }
  }
