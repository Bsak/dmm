...
GeneralUdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::GeneralUdpClient")
   ...
   .AddAttribute ("Version",
                   ...
                   StringValue (""),
                   MakeStringAccessor (&GeneralUdpClient::m_version),
                   MakeStringChecker ())
...
/*  if (m_type == 0) //Video
  {
	  ...
	  //ClientFile1 << "Video" << "_" << m_nodeId << "_" <<  "_TX_time.csv";
  }
  if (m_type == 1) //Game Uplink
    {
  	  int m_nodeId = GetNode ()->GetId ();
  	  //ClientFile1 << "Gaming_UL" << "_" << m_nodeId << "_" <<  "_TX_time.csv";
    }
  if (m_type == 2) //Game Downlink
    {
  	  int m_nodeId = GetNode ()->GetId ();
  	  //ClientFile1 << "Gaming_DL" << "_" << m_nodeId << "_" <<  "_TX_time.csv";
    }*/
  if (m_type == 3) //VoIP
      {
    	  int m_nodeId = GetNode ()->GetId ();
    	  ClientFile1 << m_version << "_Voice" << "_" << m_nodeId << "_" <<  "_TX_time.csv";
      }
	  ...
	  if (m_voipActive) // VOIP active and talk frame initialized
	  {
		  ...
		  if ((m_socket->Send (p)) >= 0)
		  {
			  ...
			  NS_LOG_INFO ("TraceDelay TX " << m_size << " bytes to "
					  ...
					  << (Simulator::Now ()).GetSeconds ());
              uint8_t buf[4];
              Ipv4Address::ConvertFrom(m_peerAddress).Serialize(buf);
              if(buf[0] != 3){
                ofstream out ((ClientFile1.str ()).c_str (), ios::app);
                            out << "TraceDelay TX " << m_size << " bytes to "
			  					  << peerAddressStringStream.str () << " Uid: "
