...
GeneralUdpServer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::GeneralUdpServer")
   ...
   .AddAttribute ("Version",
                   "Prefix of output .csv file.",
                   StringValue (""),
                   MakeStringAccessor (&GeneralUdpServer::m_version),
		   MakeStringChecker ())
  ;
  return tid;
}
...
/*  if (m_type == 0) //Video
  ...
  if (m_type == 3) //Voice Downlink
      {
    	  int m_nodeId = GetNode ()->GetId ();
    	  ClientFile1 << m_version << "_Voice" << "_" << m_nodeId << "_" <<  "_RX_time.csv";
      }
  ...
  while ((packet = socket->RecvFrom (from)))
    {
	  ...
	  if(std::find(m_receivedSeq.begin(), m_receivedSeq.end(), currentSequenceNumber) != m_receivedSeq.end()) continue;
	  m_receivedSeq.push_back(currentSequenceNumber);
	  if(m_type == 3){
	          if (InetSocketAddress::IsMatchingType (from))
        	  {
	              ...
	              out << "TraceDelay: RX " << packet->GetSize () <<
	                      ...
	                      " RXtime: " << Simulator::Now ().GetMilliSeconds() <<
        	              " TotalDelay: " << Simulator::Now ().GetMilliSeconds() - seqTs.GetTs ().GetMilliSeconds() << endl;
        	  }
	}
	else {
		if (InetSocketAddress::IsMatchingType (from))
